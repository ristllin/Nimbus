...
function applyState(d){
  ...
  if(d.cloud&&$('cloudLine')){
    ...
    if(c.state==='pairing'&&c.code){cc.style.display='block';
      cc.innerHTML='<div style="margin:2px 0 8px">Enter this code at <b>app.cumulo-nimbus.ai</b> while signed in:</div>'+
        '<div id=cloudCodeVal title="tap to copy" style="font:800 30px/1.15 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:4px;color:#67e8c3;background:#0c1a19;border:1px solid #1f3d39;border-radius:10px;padding:12px 14px;text-align:center;cursor:pointer;user-select:all"></div>';
      var _cv=$('cloudCodeVal');_cv.textContent=c.code;   // untrusted pairing code: textContent, never innerHTML
      _cv.onclick=function(){try{navigator.clipboard.writeText(c.code);toast('Code copied');}catch(e){}};}
