...
function scan(){
 ...
 const poll=()=>fetch('/api/wifi?scan=1').then(jok).then(d=>{
  ...
  // like every phone's Wi-Fi picker (CUM-233). Hidden ('') SSIDs are all kept.
  const seen={};
...
(function(){
  ...
  function wire(s){
    if(s.id==='display'){
      ...
      paint(null);
      fetch('/api/state').then(r=>r.json()).then(st=>{wifiOk=!!st.sta;provOk=!!st.provVerified;paint(st);}).catch(()=>{});
    }
  }
  function doScan(){
