...
<tr id=battRtopRow><td>Sense resistor (top)</td><td><input id=battRtop type=number min=1000 max=10000000 step=1000 style="width:110px"> &#8486; <button class=qh type=button aria-expanded=false aria-label="About the sense resistors">?</button><p class="hint tip">The two divider resistors between the pack and the ADC pin. Defaults 220k / 100k; some boards use 270k / 120k. Getting these right fixes the voltage reading. After changing them, re-run Calibrate on a full pack.</p></td></tr>
<tr id=battRbotRow><td>Sense resistor (bottom)</td><td><input id=battRbot type=number min=1000 max=10000000 step=1000 style="width:110px"> &#8486;</td></tr>
</tbody></table>
...
<p class=hint style="margin:6px 0 8px">A Cumulo Nimbus key will be created for this device with the monthly spend cap you set. It renews each billing period. You can revoke it anytime in the app.</p>
<div class=row style="align-items:center"><label for=cloudMintCap>Capacity (credits per month)</label><input type=number id=cloudMintCap min=1 step=1 value=1000 style="width:110px"><button id=cloudMintSave type=button>Save</button></div>
