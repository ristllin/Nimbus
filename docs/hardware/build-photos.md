# Build photos

A photo walk-through of an actual Nimbus build, in assembly order. These
complement the step-by-step wiring and assembly in the
[e-paper + knob](build-eink.md) and [touch TFT](build-tft.md) guides - the
electronics, power, and case steps are shared; only the final display differs.

## Printed parts and heat-set inserts

![The 3D-printed enclosure parts laid out](photos/printed-parts.webp)
![Heat-set threaded inserts pressed into the top case](photos/heat-inserts-top.webp)
![Heat-set threaded inserts pressed into the bottom case](photos/heat-inserts-bottom.webp)

## Board and custom PCB

![The ESP32-S3-DevKitC-1 N16R8 dev board](photos/esp32-s3.webp)
![Custom Nimbus carrier PCB, top side](photos/custom-pcb-top.webp)
![Custom Nimbus carrier PCB, back side](photos/custom-pcb-back.webp)
![PCB layout / silk reference](photos/pcb-layout.webp)
![Female headers fitted to the carrier board](photos/pcb-female-headers.webp)
![Modules seated on the assembled carrier board](photos/pcb-assembled.webp)
![Assembled carrier board, underside](photos/pcb-assembled-back.webp)
![Carrier board with the BMS charger and DC-DC converter](photos/pcb-charger-dcdc.webp)
![The peripheral modules before assembly](photos/modules.webp)

## LED ring, encoder, and audio

![WS2812B 45-pixel LED ring](photos/led-ring.webp)
![LED ring with one power ribbon cut for the case pass-through](photos/led-ring-cut.webp)
![EC11 encoder with jumper leads](photos/encoder-jumpers.webp)
![EC11 encoder wired to the board](photos/encoder-wired.webp)
![Full-range 4 ohm speaker](photos/speaker.webp)
![Speaker mounted in the case](photos/speaker-mounted.webp)
![Rocker switch pressed into place](photos/rocker-switch.webp)
![Longer screws for the microSD holder](photos/sd-holder-screws.webp)

## Battery pack (2S)

![Cells wrapped in fish-paper and Kapton tape](photos/cell-insulation.webp)
![The 2S BMS seated between the 18650 cells](photos/bms-between-cells.webp)
![Spot-welding the cell tabs](photos/spot-weld.webp)
![Soldering the BMS pack lead](photos/solder-pack-wire.webp)
![Battery pack lead terminal](photos/battery-lead-terminal.webp)
![Connecting the battery to the board](photos/connect-battery.webp)
![Calibrating the DC-DC converter to a clean 5 V](photos/calibrate-dcdc-5v.webp)

## Electronics assembled

![All electronics assembled](photos/electronics-stack.webp)
![All electronics assembled, other angle](photos/electronics-stack-2.webp)
![The stack fitted into the bottom case](photos/assembled-bottom-case.webp)

## Display - e-paper + knob build

![Assembled e-paper Nimbus, front](photos/eink-front.webp)
![Assembled e-paper Nimbus, top](photos/eink-top.webp)
![Assembled e-paper Nimbus, top, second view](photos/eink-top-2.webp)

## Display - touch TFT build

![2.8 inch ILI9341 touch TFT module](photos/tft-board.webp)
![Assembled touch-TFT Nimbus](photos/tft-assembled.webp)
