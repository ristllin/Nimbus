...
// So glyph pixel (col, row) is set iff kFont5x7[ch - 32][col] & (1 << row).
// ============================================================================

namespace nimbus::font {

constexpr uint8_t kFirstChar = 32;   // ' '
constexpr uint8_t kLastChar = 126;   // '~'
constexpr uint8_t kFont5x7[kLastChar - kFirstChar + 1][5] = {
...
// The glyph for `c`, falling back to '?' for anything outside the atlas.
inline const uint8_t* glyphFor(char c) {
  const uint8_t u = uint8_t(c);
  if (u < kFirstChar || u > kLastChar) return kFont5x7['?' - kFirstChar];
  return kFont5x7[u - kFirstChar];
}
