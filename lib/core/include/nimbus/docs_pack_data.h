...
// 13 files, 177 sections, ~234 KB (from 223 KB of markdown).
#pragma once
...
safety/moderation/access). Editable only by the human (web UI / provisioning).)NIMBUSDOC"},
...
9. Recent conversation (rolling, trimmed front-first when over budget;
   a summarize-and-compress step keeps the last 2 messages + a summary block).)NIMBUSDOC"},
  {"orchestrator-world#2-per-turn-context-assembly-2-1-recall", "2. Per-turn context assembly - 2.1 Recall",
   R"NIMBUSDOC(```
...
(dashboard PUT `/api/mem/config`).)NIMBUSDOC"},
  {"orchestrator-world#2-per-turn-context-assembly-2-2-maintenance", "2. Per-turn context assembly - 2.2 Maintenance",
   R"NIMBUSDOC(Periodically (each N turns / daily): `vdb.decay_importance(decay_factor)` then
...
[orchestrator-storage.md §2a](orchestrator-storage.md) and CUM-225.)NIMBUSDOC"},
  {"orchestrator-world#2-per-turn-context-assembly-2-3-rapid-message-batching-one-turn-per-burst", "2. Per-turn context assembly - 2.3 Rapid-message batching (one turn per burst)",
   R"NIMBUSDOC(When several messages arrive in one chat in quick succession (a thought split
...
batched: each still runs its own turn.)NIMBUSDOC"},
...
| `memory.archive` | Search or restore memories that expired (reached their TTL) and were moved to the SD archive instead of being deleted. | `action` (**required**, `search` \| `restore` \| `list`); `query` (string), `id` (string), `n_results` (int), `limit` (int), `ttl` (enum, on restore) | live (SD only) |)NIMBUSDOC"},
...
- `memory.archive` reaches the **cold store** of TTL-expired memories. Because
  ...
  drop at their TTL exactly as before and this tool is absent.)NIMBUSDOC"},
...
emits WAV (OpenAI) or MP3 (Mistral).)NIMBUSDOC"},
...
in `lib/core/.../connectors_wire.cpp`, host-tested so the two never drift.)NIMBUSDOC"},
...
### Telegram owner commands)NIMBUSDOC"},
  {"tools-and-commands#intro-2", "Tools & Commands (2/2)",
   R"NIMBUSDOC(In Orchestrator mode the device answers a few fixed commands over Telegram
...
`action=mode&value=ask|auto`, `action=approve&id=<id>`, or `action=cancel&id=<id>`.)NIMBUSDOC"},
...
flash and report.)NIMBUSDOC"},
  {"tools-and-commands#contributor-tools-console-commands", "Contributor tools - Console commands (1/4)",
...
#### Display & screen)NIMBUSDOC"},
  {"tools-and-commands#contributor-tools-console-commands-2", "Contributor tools - Console commands (2/4)",
...
#### Music playback)NIMBUSDOC"},
  {"tools-and-commands#contributor-tools-console-commands-3", "Contributor tools - Console commands (3/4)",
   R"NIMBUSDOC(The device plays music from the SD card's `/music` folder over the speaker, on top
...
This page is the Nimbus-side consolidated view.)NIMBUSDOC"},
  {"hardware#board-configurations", "Board configurations",
   R"NIMBUSDOC(A Nimbus device runs in **one of two display configurations**, one per board. The
...
for reflashing. The UART port remains the guaranteed path if a build ever wedges.)NIMBUSDOC"},
  {"hardware#shared-peripherals", "Shared peripherals",
   R"NIMBUSDOC(These are present on the **Solide S3** board. The display + touch pins are on the
...
> pass). Gmail, Calendar, and Drive *are* reachable.)NIMBUSDOC"},
  {"connectors#setting-up-a-connector", "Setting up a connector",
   R"NIMBUSDOC(Every connector is a **paste-a-credential card** under **Assistant →
...
glance state on a dark idle ring.)NIMBUSDOC"},
  {"modes-and-signals#2-ring-level-what-the-ring-shows-in-each-mode-the-screen-backlight", "2. Ring level - what the ring shows in each mode - The screen backlight",
   R"NIMBUSDOC(On the color touch panel the **backlight is the largest continuous draw** -
...
touch panel cannot be reached from the device menu.)NIMBUSDOC"},
...
land where you touch.)NIMBUSDOC"},
  {"modes-and-signals#2-ring-level-what-the-ring-shows-in-each-mode-restart", "2. Ring level - what the ring shows in each mode - Restart",
   R"NIMBUSDOC(**Settings > Restart** on the device (or **Restart** on the web, both in the Home
...
battery**), which offers a **Restart now** action right beside the setting.)NIMBUSDOC"},
  {"modes-and-signals#2-ring-level-what-the-ring-shows-in-each-mode-power-off", "2. Ring level - what the ring shows in each mode - Power off",
   R"NIMBUSDOC(**Settings > Power off** on the device (or **Power off** on the web, in the Home
...
applies and tells the truth on both the screen and the web page.)NIMBUSDOC"},
...
two toggles still save; they apply once a pack is fitted and monitored.)NIMBUSDOC"},
...
cue, so in Dark mode or with the ring silenced the sound and the screen carry it.)NIMBUSDOC"},
...
| **Safety activity and allowlist** | Web app: Assistant, Safety, Activity (review what the scanner blocked or suspected; approve scoped, dismiss, or report) and Allowed items (revoke) | Device menu: not shown there; the `SAFETY?` serial command prints a one-line summary. |)NIMBUSDOC"},
...
Full specification: [Notifier status language](./notifier-status-language.md).)NIMBUSDOC"},
  {"modes-and-signals#6-menu-ring-echo", "6. Menu ring echo",
   R"NIMBUSDOC(While the settings menu is open, the ring becomes a **fill bar** that echoes the
...
resumes.)NIMBUSDOC"},
  {"modes-and-signals#7-wi-fi-saved-networks-and-failover-orchestrator-only", "7. Wi-Fi: saved networks and failover (Orchestrator only)",
   R"NIMBUSDOC(Orchestrator remembers up to **five** Wi-Fi networks (home, office, a phone
...
Full detail: [Wi-Fi resilience](./wifi-resilience.md).)NIMBUSDOC"},
  // ---- led-ux ----
...
renders those decisions with transitions between them.)NIMBUSDOC"},
  {"led-ux#for-developers-architecture-and-status-candidate-working-animations", "For developers - architecture and status - Candidate \"working\" animations",
   R"NIMBUSDOC(The processing slide is the one animated steady state the ambient grammar allows, and
...
comet until a variant is chosen.)NIMBUSDOC"},
  {"led-ux#for-developers-architecture-and-status-keeping-the-head-arc-honest-wake-ups", "For developers - architecture and status - Keeping the head arc honest (wake-ups)",
   R"NIMBUSDOC(The orchestrator paints a "head" arc while a turn runs and holds it while that
...
primary path and is worth investigating.)NIMBUSDOC"},
  {"led-ux#for-developers-architecture-and-status-nothing-lit-outlives-its-job", "For developers - architecture and status - Nothing lit outlives its job",
   R"NIMBUSDOC(A turn's arcs are armed and cleared on the poll task (the head arc in the turn
...
device still reads **0**.)NIMBUSDOC"},
  // ---- memory ----
...
`mem.sfxStackMin` and `mem.musicStackMin` alongside `pollStackMin` / `asyncStackMin`.)NIMBUSDOC"},
...
chat's "Show full result" (or `GET /api/mem/blob?path=`).)NIMBUSDOC"},
  {"turn-anatomy#when-a-turn-can-t-run", "When a turn can't run",
   R"NIMBUSDOC(A turn always answers, even when it cannot reach a model. If no provider is set
...
the reply names that real cause instead. The device never goes silent on a chat.)NIMBUSDOC"},
...
- one conversation's memories are still never visible to another.)NIMBUSDOC"},
...
inline constexpr size_t kDocsSectionCount = 177;

inline constexpr DocFile kDocsFiles[] = {
  {"orchestrator-world", "Orchestrator \"World\" - memory, capability & session architecture", 0, 25},
  {"tool-catalog", "On-device tool catalog", 25, 20},
  {"tools-and-commands", "Tools & Commands", 45, 13},
  {"hardware", "Nimbus Hardware Reference", 58, 14},
  {"turn-contract", "Orchestrator Turn Contract", 72, 14},
  {"sub-sessions", "Sub-sessions - the orchestrator's background agents, end to end", 86, 12},
  {"connectors", "Connectors - external tools, per provider", 98, 16},
  {"modes-and-signals", "Modes & signals - what the settings mean and what the device is telling you", 114, 15},
  {"led-ux", "The LED ring - motion & color language", 129, 11},
  {"memory", "Memory - the two RAM pools, the turn budget, and conversation compaction", 140, 16},
  {"turn-anatomy", "Turn anatomy - what the model actually sees, and how to inspect it raw", 156, 7},
  {"people-and-privacy", "People and privacy - roles, quotas, what stays private", 163, 8},
  {"notifier-status-language", "Notifier status language", 171, 6},
};
