...
// 13 files, 162 sections, ~203 KB (from 194 KB of markdown).
#pragma once
...
- **LittleFS won't auto-create parent dirs** - the vector blob at
  ...
  {"tool-catalog#intro", "On-device tool catalog",
   R"NIMBUSDOC(The Nimbus Orchestrator exposes a single, authoritative set of tools through an
...
- [People & privacy](../people-and-privacy.md) - roles, quotas, and the RBAC
  ...
  // ---- tools-and-commands ----
  {"tools-and-commands#intro", "Tools & Commands",
   R"NIMBUSDOC(The catalog of every script, build environment, console command, and remote
...
the build - see [`tools/README.md`](../tools/README.md).)NIMBUSDOC"},
  {"tools-and-commands#user-tools", "User tools",
   R"NIMBUSDOC(### Device setup & recovery
...
| `MEDIATEST` | End-to-end test of the durable media path on the real SD card (write, capture, verify over HTTP). |)NIMBUSDOC"},
  {"tools-and-commands#contributor-tools-console-commands-3", "Contributor tools - Console commands (3/3)",
   R"NIMBUSDOC(#### Network & Bluetooth
...
[Telegram owner commands](#telegram-owner-commands) under User tools above.)NIMBUSDOC"},
  ...
  {"hardware#intro", "Nimbus Hardware Reference",
   R"NIMBUSDOC(Project-level pinout + wiring for the **Solide S3** board (ESP32-S3-DevKitC-1 N16R8).
...
This page is the Nimbus-side consolidated view.)NIMBUSDOC"},
  {"hardware#board-configurations", "Board configurations",
   R"NIMBUSDOC(A Nimbus device is built in **one of two display/input configurations**. They run the
...
Everything below this section is **common to both configurations**.)NIMBUSDOC"},
...
for reflashing. The UART port remains the guaranteed path if a build ever wedges.)NIMBUSDOC"},
  {"hardware#shared-peripherals", "Shared peripherals",
   R"NIMBUSDOC(These are present in **both** configurations. The display + input pins are
...
- architecturally incompatible with "Notifier shows live BLE-pushed status".)NIMBUSDOC"},
  ...
  {"turn-contract#intro", "Orchestrator Turn Contract",
   R"NIMBUSDOC(The **turn contract** is the JSON shape every provider-hosted LLM must return
...
| `utf8CapLen` | - | `mem_cap.h` | UTF-8-safe byte cap (drops a straddling multi-byte char whole) |)NIMBUSDOC"},
  ...
  {"sub-sessions#intro", "Sub-sessions - the orchestrator's background agents, end to end",
   R"NIMBUSDOC(What actually happens when the head model spawns a sub-agent: the exact brief it
...
- `/api/lastturn` - whether the last head turn spawned anything (section 3, the
  ...
  {"connectors#intro", "Connectors - external tools, per provider",
   R"NIMBUSDOC(Connectors let the Orchestrator (and the sub-agents it spawns) reach the
...
- **Mistral:** connect GitHub in Studio, then enable it by name here (no token
  on device).
- **Unlocks:** issues, PRs and reviews, Actions, code search, Gists, releases.)NIMBUSDOC"},
...
never places trades or moves money on its own. Keep write scopes off.)NIMBUSDOC"},
  ...
  {"modes-and-signals#intro", "Modes & signals - what the settings mean and what the device is telling you",
   R"NIMBUSDOC(Nimbus has three settings that shape everything you see and hear. Two of them
...
| Header (`ring:/sound:/power:`) | `lib/core/src/epd_screens.cpp` `drawHeader` |)NIMBUSDOC"},
  ...
  {"led-ux#intro", "The LED ring - motion & color language",
   R"NIMBUSDOC(The ring is not a status readout, it is an **ambient companion on your desk**.
...
![The ring status language: Running is a sweeping comet in the theme hue, Waiting for input breathes purple, Awaiting approval breathes amber, Error breathes the theme's alert hue, and several sessions pack the ring as arcs with dim separators](hardware/diagrams/ring-status-language.svg))NIMBUSDOC"},
...
- **Context assembly**: the anchored summary **clips** (floor 1024 B) before it
  ...
  {"turn-anatomy#intro", "Turn anatomy - what the model actually sees, and how to inspect it raw",
   R"NIMBUSDOC(Every orchestrator turn is assembled from scratch on the device. When a reply seems
...
`before_hours`); the recent window above is the auto-injected slice of it.)NIMBUSDOC"},
  {"turn-anatomy#diagnosing-a-confusing-reply", "Diagnosing a confusing reply",
   R"NIMBUSDOC(Work through the blocks above in order: `/api/lastturn` shows whether the
...
the model never has to guess one. See docs/memory.md for the gradient rules.)NIMBUSDOC"},
  ...
  {"people-and-privacy#intro", "People and privacy - roles, quotas, what stays private",
   R"NIMBUSDOC(Nimbus started as one person's device. It now talks to whoever the owner lets
...
- [Security posture](./security.md) - the device's open security items)NIMBUSDOC"},
  ...
  {"notifier-status-language#intro", "Notifier status language",
   R"NIMBUSDOC(The Notifier ring speaks a **status language**: every AI-coding-session status maps
...
inline constexpr size_t kDocsSectionCount = 162;
...
inline constexpr DocFile kDocsFiles[] = {
  ...
  {"modes-and-signals", "Modes & signals - what the settings mean and what the device is telling you", 110, 8},
  {"led-ux", "The LED ring - motion & color language", 118, 8},
  {"memory", "Memory - the two RAM pools, the turn budget, and conversation compaction", 126, 16},
  {"turn-anatomy", "Turn anatomy - what the model actually sees, and how to inspect it raw", 142, 6},
  {"people-and-privacy", "People and privacy - roles, quotas, what stays private", 148, 8},
  {"notifier-status-language", "Notifier status language", 156, 6},
};
